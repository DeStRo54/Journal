...
import { usePatchAdminRefreshAllDataMutation, usePatchAdminUpdateClassesMutation } from '@/utils/redux/apiSlices/adminApiSlice/adminApi';
...
export const AdminPanel = () => {
  ...
  const [patchAdminUpdateClassesMutation, { isLoading: updateIsLoading, isError: updateIsError }] = usePatchAdminUpdateClassesMutation();
  const [patchAdminRefreshAllDataMutation, { isLoading: refreshIsLoading, isError: refreshIsError }] = usePatchAdminRefreshAllDataMutation();
  ...
  return (
    <article className={styles.container} id='admin'>
      <Header />
      <Typhography tag="h2" variant="header" children="Панель администратора" className={clsx(styles['title'], styles['content'])} />
      <div className={styles['content']}>
        <form className={styles['form']} onSubmit={form.handleSubmit}>
          ...
          />
          <Button variant="accept" type="submit" disabled={state.isLoading} children={state.isLoading ? <Loader /> : "Добавить группу"} />
        ...
        <div className={styles['schedule']}>
          ...
          <div className={styles['schedule-item']}>
            <Button variant="accept" type="submit" disabled={updateIsLoading} children={updateIsLoading ? <Loader /> : "Обновить расписание"} onClick={updateSchedule} />
      ...
      <Modal modalId="admin" showInfo={isOpen} showDetails={onRefreshClick} >
        <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={styles['modal-admin']}>
          <Typhography tag="h3" variant="primary" children="Вы уверены?" />
          <Typhography tag="p" variant="primary" children="Все данные будут стерты" />
          <Button variant="accept" type="submit" disabled={refreshIsLoading} children={refreshIsLoading ? <Loader /> : "Стереть"} onClick={refreshData} />
